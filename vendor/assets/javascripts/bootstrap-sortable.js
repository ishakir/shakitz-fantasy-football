...
(function ($) {
    ...
    $.bootstrapSortable = function (applyLast, sign) {
        ...
        $('table.sortable').each(function () {
            ...
            $this.find('td').each(function () {
                ...
                else if($this.attr('data-recordformat') != undefined) {
                	$this.attr('data-value', record_value($this.text()));
                }
                else {
	...
	// Translate a record into a unique value for sorting
	// -((wins * 10000) + (draws * 100) + (99 - losses))
	//
	// Examples: 
	// 5-3-1 -> -50196, 5-4-1 -> -50195
	// 10-7-5 -> -100794
	//
	// This ensures that more wins is always better, followed by more draws
	// Finally if those that the same, least losses wins
	function record_value($record) {
		var recordAsString = $this.text();
        
        // Split out wins, draws, losses
        var recordAsArray = recordAsString.split("-");
        var wins = recordAsArray[0], draws = recordAsArray[2], losses = recordAsArray[1];
        
        return (wins * 10000) + (draws * 100) + (99 - losses);
