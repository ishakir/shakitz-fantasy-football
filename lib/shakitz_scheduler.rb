...
class ShakitzScheduler
 	...
 	def at(time, description)
 		if time > Time.zone.now
 			...
 			Rails.logger.info "Scheduling #{description} for #{format_for_rufus(time)} in #{Time.zone} which we converted to #{format_for_rufus(local_time)} in #{Settings.local_timezone}"
 	...
 	def convert_timezone(time)
 		return time.in_time_zone(Settings.local_timezone)
 	end
